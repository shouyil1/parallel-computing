...
pthread_mutex_t** mutexes; // Mutex array to control access to elements of matrix C
	...
	for(int i = data->start_row; i < data->end_row; i++) {
		for(int j = data->start_col; j < data->end_col; j++) {
			for(int k = 0; k < data->n; k++) { 
				pthread_mutex_lock(&mutexes[i][k]);
				data->C[i][k] += data->A[i][j] * data->B[j][k];
				pthread_mutex_unlock(&mutexes[i][k]);
			}
		}
	}
...
int main(int argc, char** argv){
		...
		for (i = 0; i < n; i++) {
			mutexes[i] = (pthread_mutex_t*) malloc(sizeof(pthread_mutex_t) * n);
			for(j = 0; j < n; j++) {
